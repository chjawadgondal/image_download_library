...
import com.app.download.util.BitmapCache;
import com.app.download.util.Common;
import com.app.download.util.RatioDrawable;
...
public class BitmapAjaxCallback extends AbstractAjaxCallback<Bitmap, BitmapAjaxCallback>{
	...
	private float anchor = Fetcher.ANCHOR_DYNAMIC;
	...
	/**
	 * Set the animation resource id, or Fetcher.FADE_IN.
	 *
	 ...
	 * 
	 * A special value Fetcher.ANCHOR_DYNAMIC will adjust the anchor base.
	...
	private static Bitmap decode(String path, byte[] data, Options options, boolean rotate){
		...
		if(result == null && options != null && !options.inJustDecodeBounds){
			FetchUtility.debug("decode image failed", path);
		}
	...
	private static boolean isInputSharable(){
		FetchUtility.debug("level", Fetcher.SDK_INT);
		return Fetcher.SDK_INT < 19;
	...
	private static Bitmap rotate(String path, Bitmap bm){
		...
		if(ori > 0){
			...
			FetchUtility.debug("before", bm.getWidth() + ":" + bm.getHeight());
			FetchUtility.debug("after", result.getWidth() + ":" + result.getHeight());
	...
	public Bitmap transform(String url, byte[] data, AjaxStatus status) {
		...
		if(bm == null){
			...
			if(fallback > 0){			
				bm = getFallback();		
			}else if(fallback == Fetcher.GONE || fallback == Fetcher.INVISIBLE){
				bm = dummy;
			}else if(fallback == Fetcher.PRESET){
				bm = preset;
			}
			...
			if(status.getSource() == AjaxStatus.NETWORK && file != null){
			    FetchUtility.debug("invalid bm from net");
			    file.delete();
			}
	...
	private void checkCb(BitmapAjaxCallback cb, String url, ImageView v, Bitmap bm, AjaxStatus status){
		...
		if(url.equals(v.getTag(Fetcher.TAG_URL))){
	...
	private static Bitmap filter(View iv, Bitmap bm, int fallback){
		...
		}else if(fallback == Fetcher.GONE){
			iv.setVisibility(View.GONE);
		}else if(fallback == Fetcher.INVISIBLE){
	...
	private void presetBitmap(String url, ImageView v){
		...
		if(!url.equals(v.getTag(Fetcher.TAG_URL)) || preset != null){
			
			v.setTag(Fetcher.TAG_URL, url);
	...
	private static boolean fadeIn(int animation, int source){
		...
		switch(animation){
			case Fetcher.FADE_IN:
				return true;
			case Fetcher.FADE_IN_FILE:
				if(source == AjaxStatus.FILE) return true;		
			case Fetcher.FADE_IN_NETWORK:
	...
	/**
	 * Fetcher internal use only. Please uses Fetcher image() methods instead.
	 * 
