...
import com.app.download.auth.AccountHandle;
import com.app.download.callback.AbstractAjaxCallback;
import com.app.download.callback.AjaxCallback;
import com.app.download.callback.BitmapAjaxCallback;
import com.app.download.callback.ImageOptions;
import com.app.download.callback.Transformer;
import com.app.download.util.FetchUtility;
import com.app.download.util.Common;
import com.app.download.util.Constants;
...
/**
 * The core class of Fetcher. Contains all the methods available from an Fetcher object.
 *
 * @param <T> the generic type
 */
public abstract class AbstractFetcher<T extends AbstractFetcher<T>> implements Constants {
	...
	/**
	 * Instantiates a new Fetcher object.
	 *
	 * @param act Activity that's the parent of the to-be-operated views.
	 */
	public AbstractFetcher(Activity act){
		this.act = act;
	}
	 ...
	 */
	public AbstractFetcher(View root){
		this.root = root;
		this.view = root;
	}
	
	/**
	 * Instantiates a new Fetcher object. This constructor should be used for Fragments.
	 *
	 * @param act Activity
	 * @param root View container that's the parent of the to-be-operated views.
	 */
	public AbstractFetcher(Activity act, View root){
		this.root = root;
		this.view = root;
		this.act = act;
	}
	...
	public AbstractFetcher(Context context){
	...
	/**
	 * Return a new Fetcher object that uses the found view as a root.
	 *
	 * @param id the id
	...
	/**
	 * Return a new Fetcher object that uses the found parent as a root.
	...
	/**
	 * Recycle this Fetcher object.
	 * 
	 * The method is designed to avoid recreating an Fetcher object repeatedly, such as when in list adapter getView method.
	 *
	 * @param root The new root of the recycled Fetcher.
	 * @return self
	 */
	...
	public T image(int resid){
		...
		if(view instanceof ImageView){
			...
			iv.setTag(Fetcher.TAG_URL, null);
			if(resid == 0){
	 ...
	 * @param animId Apply this animation when image is loaded. 0 = no animation. Also accept Fetcher.FADE_IN as a predefined 500ms fade in animation.
	 * @return self
	 * 
	 * @see testImage7
	 * 
	 ...
	 * @param preset Default image to show before real image loaded. null = no preset.
	 * @param animId Apply this animation when image is loaded. 0 = no animation. Also accept Fetcher.FADE_IN as a predefined 500ms fade in animation.
	 * @param ratio The desired aspect ratio of the imageview. Ratio is height / width, or Fetcher.RATIO_PRESERVE to preserve the original aspect ratio of the image.
	 * @return self
	 * 
	 * @see testImage12
	 * 
	...
	protected T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animId, float ratio, int round, String networkUrl){
		...
		if(view instanceof ImageView){		
			BitmapAjaxCallback.async(act, getContext(), (ImageView) view, url, memCache, fileCache, targetWidth, fallbackId, preset, animId, ratio, Fetcher.ANCHOR_DYNAMIC, progress, ah, policy, round, proxy, networkUrl);
			reset();
		}
	...
	public T transparent(boolean transparent){
		...
		if(view != null){
			FetchUtility.transparent(view, transparent);
		}
	...
	private Common setScrollListener(){
		...
		Common common = (Common) lv.getTag(Fetcher.TAG_SCROLL_LISTENER);
		if(common == null){
			common = new Common();
			lv.setOnScrollListener(common);
			lv.setTag(Fetcher.TAG_SCROLL_LISTENER, common);
			FetchUtility.debug("set scroll listenr");
		}
	...
	public T overridePendingTransition5(int enterAnim, int exitAnim){
		...
		if(act != null){
			FetchUtility.invokeHandler(act, "overridePendingTransition", false, false, PENDING_TRANSITION_SIG, enterAnim, exitAnim);
		}
	 ...
	 *
	 * @param mode Fetcher.OVER_SCROLL_ALWAYS, Fetcher.OVER_SCROLL_ALWAYS, Fetcher.OVER_SCROLL_IF_CONTENT_SCROLLS
	 * @return self
	 */
	public T setOverScrollMode9(int mode){
		
		if(view instanceof AbsListView){
			FetchUtility.invokeHandler(view, "setOverScrollMode", false, false, OVER_SCROLL_SIG, mode);
		}
	 ...
	 *
	 * Type must be Fetcher.LAYER_TYPE_SOFTWARE or Fetcher.LAYER_TYPE_HARDWARE.
	 * 
	 * @param type the type
	...
	public T setLayerType11(int type, Paint paint){
		...
		if(view != null){
			...
			FetchUtility.invokeHandler(view, "setLayerType", false, false, LAYER_TYPE_SIG, type, paint);
		}
	...
	public Object invoke(String method, Class<?>[] sig, Object... params){
		...
		return FetchUtility.invokeHandler(handler, method, false, false, sig, params);
	}
	...
	public T hardwareAccelerated11(){
		...
		if(act != null){
			act.getWindow().setFlags(Fetcher.FLAG_HARDWARE_ACCELERATED, Fetcher.FLAG_HARDWARE_ACCELERATED);
		}
	...
	public T clear(){
		...
		if(view != null){
			...
			if(view instanceof ImageView){
				...
				wv.stopLoading();
				wv.clearView();
				wv.setTag(Fetcher.TAG_URL, null);
	...
	public T margin(float leftDip, float topDip, float rightDip, float bottomDip){
		...
		if(view != null){
			...
			if(lp instanceof MarginLayoutParams){
				...
				int left = FetchUtility.dip2pixel(context, leftDip);
				int top = FetchUtility.dip2pixel(context, topDip);
				int right = FetchUtility.dip2pixel(context, rightDip);
				int bottom = FetchUtility.dip2pixel(context, bottomDip);
	...
	private void size(boolean width, int n, boolean dip){
		...
		if(view != null){
			...
			if(n > 0 && dip){
				n = FetchUtility.dip2pixel(context, n);
			}
	...
	public <K> T delete(String url, Class<K> type, AjaxCallback<K> callback){
		...
		callback.url(url).type(type).method(Fetcher.METHOD_DELETE);
		return ajax(callback);
	...
	public <K> T put(String url, String contentHeader, HttpEntity entity, Class<K> type, AjaxCallback<K> callback){
		...
		callback.url(url).type(type).method(Fetcher.METHOD_PUT).header("Content-Type", contentHeader).param(Fetcher.POST_ENTITY, entity);
		return ajax(callback);
	...
	public <K> T post(String url, String contentHeader, HttpEntity entity, Class<K> type, AjaxCallback<K> callback){
        ...
        callback.url(url).type(type).method(Fetcher.METHOD_POST).header("Content-Type", contentHeader).param(Fetcher.POST_ENTITY, entity);
        return ajax(callback);
	...
	public File getCachedFile(String url){
		...
		//return FetchUtility.getExistedCacheByUrl(FetchUtility.getCacheDir(getContext()), url);
		File result = FetchUtility.getExistedCacheByUrl(FetchUtility.getCacheDir(getContext(), Fetcher.CACHE_PERSISTENT), url);
		if(result == null) result = FetchUtility.getExistedCacheByUrl(FetchUtility.getCacheDir(getContext(), Fetcher.CACHE_DEFAULT), url);
		return result;
	}
	 ...
	 * 
	 * <br>
	 * The temp file will be deleted when FetchUtility.cleanCacheAsync is invoked, or the file can be explicitly deleted after use.
	 * 
	...
	public File makeSharedFile(String url, String filename){
		...
		try{
			...
			if(cached != null){
				...
				File temp = FetchUtility.getTempDir();
				...
				if(temp != null){
				    ...
				    try{
				    	...
				    	FetchUtility.close(fis);
				    	FetchUtility.close(fos);
				    	FetchUtility.close(ic);
				    	FetchUtility.close(oc);
				    }
	...
	public View inflate(View convertView, int layoutId, ViewGroup root){
		...
		if(convertView != null){
			Integer layout = (Integer) convertView.getTag(Fetcher.TAG_LAYOUT);
		...
		view.setTag(Fetcher.TAG_LAYOUT, layoutId);
