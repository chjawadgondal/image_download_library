...
/**
 * Fetcher internal use only. A shared listener class to reduce the number of classes.
 * 
 */
...
public class Common implements Comparator<File>, Runnable, OnClickListener, OnLongClickListener, OnItemClickListener, OnItemLongClickListener, OnScrollListener, OnItemSelectedListener, TextWatcher{
	...
	private Object invoke(Object... args){
		...
		if(method != null){
			...
			Object result = FetchUtility.invokeHandler(cbo, method, fallback, true, sig, input);
			return result;
		}else if(methodId != 0){
			...
			switch(methodId){
				...
				case CLEAN_CACHE:
					FetchUtility.cleanCache((File) params[0], (Long) params[1], (Long) params[2]);
					break;
				case STORE_FILE:
					FetchUtility.store((File) params[0], (byte[]) params[1]);
					break;
	...
	private void onScrollStateChanged(ExpandableListView elv, int scrollState){
		...
		elv.setTag(Fetcher.TAG_NUM, scrollState);
		...
		if(scrollState == SCROLL_STATE_IDLE){
			...
			for(int i = 0; i <= count; i++){
				...
				if(group >= 0){
					...
					Long targetPacked = (Long) convertView.getTag(Fetcher.TAG_NUM);
					...
					if(targetPacked != null && targetPacked.longValue() == packed){
						...
						convertView.setTag(Fetcher.TAG_NUM, null);
					}else{
						//FetchUtility.debug("skip!");
					}
	...
	private void onScrollStateChanged2(AbsListView lv, int scrollState){
		...
		lv.setTag(Fetcher.TAG_NUM, scrollState);
		...
		if(scrollState == SCROLL_STATE_IDLE){
			...
			for(int i = 0; i <= count; i++){
				...
				Number targetPacked = (Number) convertView.getTag(Fetcher.TAG_NUM);
	...
	public static boolean shouldDelay(int groupPosition, int childPosition, View convertView, ViewGroup parent, String url){
		...
		OnScrollListener sl = (OnScrollListener) parent.getTag(Fetcher.TAG_SCROLL_LISTENER);
		
		if(sl == null){
			sl = new Common();
			lv.setOnScrollListener(sl);
			parent.setTag(Fetcher.TAG_SCROLL_LISTENER, sl);
		}
		
		Integer scrollState = (Integer) lv.getTag(Fetcher.TAG_NUM);
		...
		convertView.setTag(Fetcher.TAG_NUM, packed);
	...
	private static boolean shouldDelayGallery(int position, View convertView, ViewGroup parent, String url){
		...
		Integer selected = (Integer) gallery.getTag(Fetcher.TAG_NUM);
		
		if(selected == null){
			
			selected = 0;
			gallery.setTag(Fetcher.TAG_NUM, 0);
		...
		if((position >= from && position <= to)){
			...
			//FetchUtility.debug("yes", position + ":" + from + "." + to);
			convertView.setTag(Fetcher.TAG_NUM, position);
			
			return false;
		}
		
		//FetchUtility.debug("no", position + ":" + from + "." + to);
		convertView.setTag(Fetcher.TAG_NUM, null);
		return true;
	...
	public void onItemSelected(AdapterView<?> parent, View v, int pos, long id) {
		...
		if(galleryListen){
			...
			Integer selected = (Integer) parent.getTag(Fetcher.TAG_NUM);
			
			if(selected != pos){
			
				Adapter adapter = parent.getAdapter();
				parent.setTag(Fetcher.TAG_NUM, pos);
			
				int count = parent.getChildCount();
				
				//FetchUtility.debug("redrawing", count);
				...
				for(int i = 0; i < count; i++){
					...
					Integer lastDrawn = (Integer) convertView.getTag(Fetcher.TAG_NUM);
					
					if(lastDrawn != null && lastDrawn.intValue() == drawPos){
						//FetchUtility.debug("skip", drawPos);
					}else{						
						//FetchUtility.debug("redraw", drawPos);
		...
		if(p != null){
			...
			if(p instanceof View){				
				...
				if(show){
					...
					Object tag = pv.getTag(Fetcher.TAG_URL);
					if(tag == null || tag.equals(url)){
						pv.setTag(Fetcher.TAG_URL, null);
				...
				Fetcher aq = new Fetcher(pd.getContext());
